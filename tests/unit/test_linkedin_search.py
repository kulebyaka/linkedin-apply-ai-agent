...
class TestBuildURLFromSettings:
    """Test LinkedInSearchURLBuilder.build_url_from_settings."""

    def test_with_mock_settings(self):
        class MockSettings:
            linkedin_search_keywords = "python"
            linkedin_search_location = "Berlin"
            linkedin_search_remote_filter = "remote"
            linkedin_search_date_posted = "week"
            linkedin_search_experience_level = ["entry", "mid-senior"]
            linkedin_search_job_type = ["full-time"]
            linkedin_search_easy_apply_only = True
            linkedin_search_max_jobs = 30

        url = LinkedInSearchURLBuilder.build_url_from_settings(MockSettings(), page=1)
        qs = _parse_url(url)
        assert qs["keywords"] == ["python"]
        assert qs["location"] == ["Berlin"]
        assert qs["f_WT"] == ["2"]
        assert qs["f_TPR"] == ["r604800"]
        assert qs["f_E"] == ["2,4"]
        assert qs["f_JT"] == ["F"]
    ...
    def test_with_empty_settings(self):
        class MockSettings:
            ...
            linkedin_search_remote_filter = None
            linkedin_search_date_posted = None
            linkedin_search_experience_level = None
            linkedin_search_job_type = None
            linkedin_search_easy_apply_only = False
            linkedin_search_max_jobs = 50

        url = LinkedInSearchURLBuilder.build_url_from_settings(MockSettings())
        qs = _parse_url(url)
        assert qs == {}
