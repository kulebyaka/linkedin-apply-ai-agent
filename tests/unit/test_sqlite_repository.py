...
async def test_operations_fail_without_initialize(tmp_path):
    ...
    job = JobRecord(
        job_id="test-1", user_id=TEST_USER_ID, source="manual", mode="mvp", status="queued"
    )
...
async def test_create_duplicate_raises_error(temp_db):
    ...
    job = JobRecord(
        job_id="dup-1", user_id=TEST_USER_ID, source="manual", mode="mvp", status="queued"
    )
...
async def test_update(temp_db):
    ...
    job = JobRecord(
        job_id="test-2", user_id=TEST_USER_ID, source="url", mode="full", status="pending"
    )
    await temp_db.create(job)
...
@pytest.mark.asyncio
async def test_pending_questions_round_trip(temp_db):
    """pending_questions persist as JSON and hydrate back into PendingQuestion."""
    from src.models.unified import PendingQuestion

    job = JobRecord(
        job_id="pq-1", user_id=TEST_USER_ID, source="linkedin", mode="full", status="approved"
    )
    await temp_db.create(job)

    questions = [
        PendingQuestion(
            selector="#shift", label="Shift", field_type="select", options=["Day", "Night"]
        ),
        PendingQuestion(
            selector="#yrs", label="Years", field_type="number", kind="years_experience"
        ),
    ]
    await temp_db.update(
        "pq-1",
        {
    ...
    updated = await temp_db.get("pq-1")
    assert updated.status == "manual_required"
    assert updated.pending_questions is not None
    assert len(updated.pending_questions) == 2
    assert updated.pending_questions[0].options == ["Day", "Night"]
    assert updated.pending_questions[1].kind == "years_experience"


@pytest.mark.asyncio
async def test_claim_manual_required_for_apply(temp_db):
    """A manual_required job is claimable for re-apply and clears pending_questions."""
    job = JobRecord(
        job_id="pq-claim",
        user_id=TEST_USER_ID,
        source="linkedin",
    ...
    await temp_db.update(
        ...
        {
            "status": "manual_required",
            "pending_questions": [{"selector": "#q", "label": "Q", "field_type": "text"}],
        },
    )

    claimed = await temp_db.try_claim_for_apply("pq-claim")

    assert claimed is not None
    assert claimed.status == "applying"
    assert claimed.pending_questions is None
...
async def test_update_multiple_fields(temp_db):
    ...
    job = JobRecord(
        job_id="test-3", user_id=TEST_USER_ID, source="url", mode="full", status="pending"
    )
    await temp_db.create(job)
...
async def test_update_invalid_field_raises_error(temp_db):
    ...
    job = JobRecord(
        job_id="test-4", user_id=TEST_USER_ID, source="url", mode="full", status="pending"
    )
    await temp_db.create(job)
...
async def test_get_for_user_returns_owned_job(temp_db):
    ...
    job = JobRecord(
        job_id="own-1", user_id=TEST_USER_ID, source="manual", mode="mvp", status="queued"
    )
    await temp_db.create(job)
...
async def test_get_for_user_returns_none_for_other_user(temp_db):
    ...
    job = JobRecord(
        job_id="own-2", user_id=TEST_USER_ID, source="manual", mode="mvp", status="queued"
    )
    await temp_db.create(job)
...
async def test_get_pending(temp_db):
    ...
    await temp_db.create(
        JobRecord(job_id="p1", user_id=TEST_USER_ID, source="url", mode="full", status="pending")
    )
    await temp_db.create(
        JobRecord(job_id="p2", user_id=TEST_USER_ID, source="url", mode="full", status="pending")
    )
    await temp_db.create(
        JobRecord(job_id="c1", user_id=TEST_USER_ID, source="url", mode="mvp", status="completed")
    )
    await temp_db.create(
        JobRecord(job_id="p3", user_id="other-user", source="url", mode="full", status="pending")
    )
...
async def test_get_by_status(temp_db):
    ...
    await temp_db.create(
        JobRecord(job_id="j1", user_id=TEST_USER_ID, source="url", mode="full", status="pending")
    )
    await temp_db.create(
        JobRecord(job_id="j2", user_id=TEST_USER_ID, source="url", mode="full", status="pending")
    )
    await temp_db.create(
        JobRecord(job_id="j3", user_id=TEST_USER_ID, source="url", mode="full", status="approved")
    )
    await temp_db.create(
        JobRecord(job_id="j4", user_id="other-user", source="url", mode="full", status="pending")
    )
...
async def test_get_all(temp_db):
    ...
    await temp_db.create(
        JobRecord(job_id="j2", user_id=TEST_USER_ID, source="url", mode="mvp", status="completed")
    )
    await temp_db.create(
        JobRecord(
            job_id="j3", user_id="other-user", source="manual", mode="full", status="declined"
        )
    )
...
async def test_get_history(temp_db):
    ...
    await temp_db.create(
        JobRecord(job_id="j1", user_id=TEST_USER_ID, source="url", mode="full", status="applied")
    )
    await temp_db.create(
        JobRecord(job_id="j2", user_id=TEST_USER_ID, source="url", mode="full", status="declined")
    )
    await temp_db.create(
        JobRecord(job_id="j3", user_id="other-user", source="url", mode="full", status="pending")
    )
...
async def test_get_history_with_status_filter(temp_db):
    ...
    await temp_db.create(
        JobRecord(job_id="j3", user_id=TEST_USER_ID, source="url", mode="full", status="failed")
    )
