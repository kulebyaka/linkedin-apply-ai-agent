...
- [x] Update `CLAUDE.md`: reflect new architecture (AppContext DI, domain services, state machine, async workflows), update directory structure section, update implementation status table, remove references to legacy endpoints
- [x] Update `CLAUDE.md` common tasks sections: "Adding a New Workflow Step" should reference `_shared.py` and async node pattern
