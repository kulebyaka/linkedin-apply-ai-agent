...
- [x] Update CLAUDE.md: mark LinkedIn Job Source Adapter as complete, update implementation status table, add `linkedin_scraper.py`, `linkedin_search.py`, `job_queue.py`, `scheduler.py` to directory structure
- [x] Update `.env.example` (or create if missing) with new LinkedIn search settings
